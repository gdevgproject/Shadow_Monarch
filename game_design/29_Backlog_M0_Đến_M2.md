...
| M0-03 | `core` có module boundary, contract package và event envelope không phụ thuộc gameplay | 16.2–3.2, 28 | M0-02 | architecture test/compile chứng minh client không vào server implementation |
| M0-04 | JSON definition mẫu hợp lệ load được; mẫu lỗi bị báo chính xác và không crash game | 15, 16.3.1, 18.2 | M0-03 | validator test + 10 valid/invalid fixtures |
...
| ID | Mục tiêu quan sát được | Nguồn | Depends | Proof/Done |
|---|---|---|---|---|
| M2-01 | Gate definition + state machine `OPEN → IN_PROGRESS → CLEARED_AWAITING_EXIT → CLOSED` hoạt động với objective giả | 08.2.1, 14.21, 15.6, 16.3.5, R01 | M0 gate | toàn bộ table Gate test 18.10 xanh |
...
| M2-04 | Objective ledger giữ quái đã hạ, loot và tiến độ khi rời Gate dở dang | 08.2, 15.6, 18.10, R01–R03 | M2-01 | leave/reload/return GameTest xanh |
...
| M2-06 | Loot/rarity cơ bản được grant server-side sau clear; exit UI nói đúng hậu quả Gate | 11, 25, R01, R16 | M2-04 | client/server test + manual exit proof |
...
| M2-08 | Playtest 20 phút với tối thiểu 5 người và retro scope M3 | 18.5, 20.M2, R24 | M2-01..07 | kết quả, issue list, quyết định go/no-go |
